...
class MultilingualBehavior extends CActiveRecordBehavior
{
    ...
    private function createLocalizedRelation($owner, $lang)
    {
        ...
        $owner->getMetaData()->relations[$this->localizedRelation] = new $class($this->localizedRelation,
            $this->langClassName, $this->langForeignKey,
            array( 'on' => $this->localizedRelation . "." . $this->langField . "='" . $lang . "'",
            'index' => $this->langField ));
    }
    ...
    public function attach($owner)
    {
        ...
        $simple_table_name = str_replace(array( '{{', '}}' ), '',
            array_pop($table_name_chunks));
        ...
        $owner->getMetaData()->relations[$this->multilangRelation] = new $class($this->multilangRelation,
            $this->langClassName, $this->langForeignKey,
            array( 'index' => $this->langField ));
            ...
            foreach($this->localizedAttributes as $attr)
            {
                foreach($rules as $rule)
                {
                    ...
                    if (in_array($attr, $rule_attributes))
                    {
                        ...
                        {
                            $validators->add(CValidator::createValidator($rule[1],
                                    $owner, $attr . '_' . $l,
                                    array_slice($rule, 2)));
                        }
                        else if ($rule[1] === 'required')
                        {
                            ...
                            $validators->add(CValidator::createValidator('safe',
                                    $owner, $attr . '_' . $l,
                                    array_slice($rule, 2)));
                        }
                    }
    ...
    {
        $owner = $this->getOwner();
        if ($lang != null && $lang != Yii::app()->language && in_array($lang,
                $this->languages))
        {
        ...
        if ($owner->scenario == $this->createScenario)
        {
            ...
            foreach($this->languages as $lang)
            {
                foreach($this->localizedAttributes as $field)
                {
                    ...
                    $this->setLangAttribute($field . '_' . $lang,
                        $owner->$ownerfield);
                }
            }
        }
    ...
    public function modifySearchCriteria(CDbCriteria $criteria)
    {
        ...
        foreach($this->localizedAttributes as $attribute)
        {
            ...
            {
                $criteria_array['condition'] = str_replace($attribute . ' ',
                    $this->localizedPrefix . $attribute . ' ',
                    $criteria_array['condition']);
            }
        }
    ...
    public function afterFind($event)
    {
        ...
        if ($owner->hasRelated($this->multilangRelation))
        {
            ...
            foreach($this->languages as $lang)
                foreach($this->localizedAttributes as $field)
                    $this->setLangAttribute($field . '_' . $lang,
                        isset($related[$lang][$this->localizedPrefix . $field]) ? $related[$lang][$this->localizedPrefix . $field]
                                : null);
        }
    ...
    public function afterDelete($event)
    {
        if ($this->forceDelete)
        {
            ...
            $model   = call_user_func(array( $this->langClassName, 'model' ));
            $model->deleteAll("{$this->langForeignKey}=:id",
                array( 'id' => $ownerPk ));
        }
    }
